...
function prestoHourRange(range, interval, currentDate) {
  ...
  var now = dayjs(currentDate).hour(startHour)
  var startTime =
...
function formatEventData(data, hoursRange, interval) {
  var result = hoursRange.map(function (timeObj) {
    ...
    data.map(function (item) {
      var startDate = dayjs(item.startDate)
      var endDate = dayjs(item.endDate)
      var startMinute = dayjs(item.startDate).get('minute')
      var endMinute = dayjs(item.endDate).get('minute')
      ...
      var diff = dayjs(item.endDate).diff(startDate, 'minutes')
...
var HourGuideColumn$1 = React.memo(_HourGuideColumn$1, function () {
  return true
})
        ...
        showHourGuide
          ? React.createElement(
              ...
              hoursRange(hourRange).map(function (hour) {
                return React.createElement(HourGuideColumn$1, {
                  key: hour,
      ...
      ? React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              View,
              null,
              React.createElement(Text, { style: { color: theme.palette.headingColor } }, '< PREV'),
            ),
          ),
          ...
          React.createElement(
            ...
            React.createElement(View, null, React.createElement(Text, null, 'NEXT >')),
          ),
        )
      : null,
  )
...
function _CalendarContainer(_a) {
  var _b = _a.events,
    ...
    multipleColumnData = _a.multipleColumnData,
    numberOfColumn = _a.numberOfColumn
  var _3 = React__default.useState(dayjs(date)),
    targetDate = _3[0],
    setTargetDate = _3[1]
  React__default.useEffect(
    function () {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      event.data && event.data.length === 0
        ? React.createElement(Text, null, 'Available for booking')
        : null,
    ),
  )
}
...
function PrestoCalendar$1(_a) {
  var headerComponent = _a.headerComponent,
    ...
    _c = _a.currentDate,
    currentDate = _c === void 0 ? dayjs().startOf('d').toISOString() : _c,
    _d = _a.renderCell,
  ...
  var date = dayjs(currentDate).startOf('d')
  var data = formatEventData(eventData, hoursRangeArr, interval)
  var getEventCellPositionStyle = function (start, end) {
    var relativeHeight = (dayjs(end).diff(start, 'minute') * cellHeight) / interval
    var relativeTop =
    ...
    return React__default.createElement(
      ...
      renderEvent
        ...
        : React__default.createElement(
            View,
            { style: { padding: 5 } },
            React__default.createElement(Text, { style: eventTimeStyle }, event.name),
            React__default.createElement(
              Text,
  ...
  return React__default.createElement(
    ...
    null,
    headerComponent != null
      ? React__default.createElement(View, { style: headerComponentStyle }, headerComponent)
      : null,
    React__default.createElement(
      ScrollView,
      {
      ...
      React__default.createElement(
        ...
        React__default.createElement(
          ...
          hoursRangeArr.map(function (item) {
            return React__default.createElement(HourGuideColumn, {
              key: item.startTime,
              cellHeight: cellHeight,
          ...
          mode === 'detailed'
            ...
            : data.map(function (event, index) {
                return React__default.createElement(
                  ...
                  renderCell
                    ? renderCell(event)
                    : React__default.createElement(DefaultEventRenderer, {
                        event: event,
                        cellHeight: cellHeight,
                      }),
                )
            ...
            ? data.map(function (event, index) {
                return React__default.createElement(
                  ...
                  renderEmptySlots
                    ...
                    : React__default.createElement(DefaultEmptySlotRenderer, {
                        event: event,
                        cellHeight: cellHeight,
